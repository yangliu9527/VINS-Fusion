...
void Estimator::processMeasurements()
{
    while (1)
    {
        ...
        if (!featureBuf.empty())
        {
            ...
            processImage(feature.second, feature.first); //***入口函数，输入最buffer里最老的帧，状态估计都在这函数里面开始进行了
            prevTime = curTime;
...
void Estimator::initFirstIMUPose(vector<pair<double, Eigen::Vector3d>> &accVector)
{
    ...
    // 仅修正偏航角方向的旋转，将初始姿态与平均加速度方向的偏航角对齐
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    // 将特征推入feature_manager
    // 检查特征的视差来判断边缘化情况（判断当前帧是否为加入滑窗的关键帧）
    if (f_manager.addFeatureCheckParallax(frame_count, image, td)) 
    {
        marginalization_flag = MARGIN_OLD; // 要是当前帧特征质量好，就优化完后将最老的关键帧边缘化掉
        // printf("keyframe\n");
    }
    else
    {
        marginalization_flag = MARGIN_SECOND_NEW; // 要是当前帧特征质量不怎么地，就优化完后将边缘化新的帧
        // printf("non-keyframe\n");
    }
...
/// @brief 初始化结构，sfm出一些初始姿态、位置和点的位置以及IMU初始化
/// @return 只有SFM和IMU都初始化成功了才会返回true
bool Estimator::initialStructure()
{
    TicToc t_sfm;
    // check imu observibility
    // 先检查IMU可观性，但这段代码算了半天最后也没起作用，没为下面的sfm做什么。直接看下面sfm的代码吧
    {
        map<double, ImageFrame>::iterator frame_it;
        Vector3d sum_g;
        // 遍历现有的所有帧
